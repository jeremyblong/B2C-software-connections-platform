...
mongo.connect(config.get("mongoURI"),  { useNewUrlParser: true }, { useUnifiedTopology: true }, cors(), (err, db) => {
	router.post('/', (req, res) => {
	    ...
	    const { amount, username, crypto_total } = req.body;

        req.flash('amount', amount);
        
        req.flash("username", username);

        req.flash("crypto_passover", crypto_total)
		...
		const create_payment_json = {
		    ...
		    "redirect_urls": {
		        "return_url": "http://localhost:5000/purchase/crypto/coins/paypal/success",
		        "cancel_url": "http://localhost:5000/purchase/crypto/coins/paypal/cancel"
		    },
		    "transactions": [{
		        "item_list": {
		            "items": [{
